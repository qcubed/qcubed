<?php
	 ...
	 * listing a collection of Comment objects.  It includes
	 ...
	 *
	 */
	class CommentDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Comment datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Comment object, or it can be any QQNode extending from QQN::Comment().
		 *
		 ...
		 * sorting using two fields from the Comment object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Comment
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Comment::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Comment, given the clauses above
			$this->DataSource = Comment::QueryArray($objConditions, $objClauses);
		}

		/**
		 * Used internally by the Meta-based Add Column tools.
		 *
		 * Given a QQNode or a Text String, this will return a Comment-based QQNode.
		 * It will also verify that it is a proper Comment-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'comment') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "comment".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Comment()->Id;
				case 'PostId': return QQN::Comment()->PostId;
				case 'Post': return QQN::Comment()->Post;
				case 'CommentBody': return QQN::Comment()->CommentBody;
