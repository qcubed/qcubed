<?php
	 ...
	 * listing a collection of PersonWithLock objects.  It includes
	 ...
	 *
	 */
	class PersonWithLockDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this PersonWithLock datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the PersonWithLock object, or it can be any QQNode extending from QQN::PersonWithLock().
		 *
		 ...
		 * sorting using two fields from the PersonWithLock object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from PersonWithLock
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = PersonWithLock::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from PersonWithLock, given the clauses above
			$this->DataSource = PersonWithLock::QueryArray($objConditions, $objClauses);
		}

		/**
		 * Used internally by the Meta-based Add Column tools.
		 *
		 * Given a QQNode or a Text String, this will return a PersonWithLock-based QQNode.
		 * It will also verify that it is a proper PersonWithLock-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'person_with_lock') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "person_with_lock".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::PersonWithLock()->Id;
				case 'FirstName': return QQN::PersonWithLock()->FirstName;
				case 'LastName': return QQN::PersonWithLock()->LastName;
				case 'SysTimestamp': return QQN::PersonWithLock()->SysTimestamp;
				default: throw new QCallerException('Simple Property not found in PersonWithLockDataGrid content: ' . $mixContent);
