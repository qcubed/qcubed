<?php
	 ...
	 * listing a collection of Post objects.  It includes
	 ...
	 *
	 */
	class PostDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Post datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Post object, or it can be any QQNode extending from QQN::Post().
		 *
		 ...
		 * sorting using two fields from the Post object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Post
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Post::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Post, given the clauses above
			$this->DataSource = Post::QueryArray($objConditions, $objClauses);
		}

		/**
		 * Used internally by the Meta-based Add Column tools.
		 *
		 * Given a QQNode or a Text String, this will return a Post-based QQNode.
		 * It will also verify that it is a proper Post-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'post') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "post".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Post()->Id;
				case 'Title': return QQN::Post()->Title;
				case 'Body': return QQN::Post()->Body;
				default: throw new QCallerException('Simple Property not found in PostDataGrid content: ' . $mixContent);
