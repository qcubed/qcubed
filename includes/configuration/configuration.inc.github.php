...
/**
 * Sample Configuration File for GitHub installation
 * 
 * This config file will is an example setup the lets you place the entire QCubed Github repository in a single
 * directory. It is meant as a stopgap measure as we move QCubed to Github. This structure will
 * allow you to easily upgrade your QCubed installation with whatever branch on github you wish 
 * to use, and will allow you to more easily contribute to the QCubed core files.
 * 
 * In order to implement this file, you will need to do the following:
 * 1) Clone a copy of the QCubed repository into a local directory that is accessible by your webserver.
 *    The expected default name below is 'framework', but you can change the name to whatever you wish
 *    by changing the defines below. This directory could be your doc_root, or a subdirectory of doc_root
 * 2) Create the following directories at the same level as the 'framework' directory (i.e. siblings of 'framework').
 *    These directories should be writable by the Web server:
 *    - project
 *    - generated
 *    - assets
 *    - tmp
 * 3) In the 'generated' directory, create the following writable directories:
 *   - formbase_classes
 *   - meta_controls
 *   - model
 * 4) Copy the following directories to the 'generated' directory:
 *   - /framework/includes/qcubed/plugins
 *   - /framework/drafts
 * 5) In the 'assets' directory, create the following writable directory:
 *    - plugins
 *    You may optionally create css, fonts, images, js and php directories here as well
 * 6) In the 'project' directory, create an 'includes' directory, and copy the following directories to 
 *    that directory so that they are subdirectories of /project/includes/:
 *   - /framework/includes/app_includes
 *   - /framework/includes/configuration
 *   - /framework/includes/qcubed/controls
 *   - /framework/includes/qcubed/framework
 *   - /framework/includes/qcubed/i18n
 * 7) In /project/includes, also create the following directories:
 *   - meta_controls
 *   - model
 * 8) In /tmp, copy all the directories of /framework/includes/tmp
 * 9) Configure the __DOCROOT__, __VIRTUAL_DIRECTORY__, and __SUBDIRECTORY__ settings below.
 * 10) Setup your database connection settings below.
 * 11) Save this file as configuration.inc.php in the /project/includes/configuration directory
 * 12) Rename /framework/includes/configuration/configuration.inc.github2.php to configuration.inc.php 
 *     and leave it in the directory you found it in.
 * 13) Copy the qcubed.inc.php file from the 'framework' directory and put it in the
 *     'generated" directory. Open it, and change the $configPath variable to read the following:
 *     		$configPath = "../../project/includes/configuration";
 * 14) Point your browser to <__SUBDIRECTORY__>/framework/assets/_core/php/_devtools/config_checker.php and you should see the startup screen.
 */
...
if (!defined('SERVER_INSTANCE')) {
	...
	switch (SERVER_INSTANCE) {
		...
		case 'stage':
			 ...
			 */
			define ('__DOCROOT__', 'C:/htdocs');	// Directory path to the web server's docroot
			define ('__VIRTUAL_DIRECTORY__', ''); // Additional directory to add to the url to get to the docroot
			define ('__SUBDIRECTORY__', '/reorg'); // Subdirectory in docroot. Gets added to both the url and file paths.
			...
			/*
			 * Top level internal directories. These directories are absolute paths, and are accessed by
			 * php code. You may relocate these anywhere in your directory tree, including outside
			 * the web server accessible directories.
			 */
			 
			define ('__PROJECT__', __DOCROOT__ . __SUBDIRECTORY__  . '/project');
			define ('__GENERATED__', __DOCROOT__ . __SUBDIRECTORY__  . '/generated');
			define ('__TMP__', __DOCROOT__ . __SUBDIRECTORY__  . '/tmp');
			define ('__QCUBED__', __PROJECT__ . '/includes'); // this is hardcoded in app_includes. Should probably be reconfigured in the core eventually
			define ('__QCUBED_2__', __DOCROOT__ . __SUBDIRECTORY__  . '/framework');

			/*
			* Top level public directories. These are relative paths from DOCROOT, and will be used
			* to generate URLs in the browser. The files in these directories must be accessible by 
			* the web server.
			*/
			
			define ('__PROJECT_ASSETS__', __SUBDIRECTORY__ . '/project/assets');
			define ('__QCUBED_ASSETS__', __SUBDIRECTORY__ . '/framework/assets/_core');
			...
			define ('__QI18N_PO_PATH__', __INCLUDES__ . '/i18n'); // Location of .po translation files
			 
			/*
			* QCubed Directories
			*/ 
			
			define ('__EXTERNAL_LIBRARIES__', __QCUBED_2__ . '/includes/external_libraries');
			define ('__QCUBED_CORE__', __QCUBED_2__ . '/includes/qcubed/_core');

			/*
			 * Destinations for generated files
			 */
			 
			define ('__MODEL__', __INCLUDES__ . '/model' );	// generated one time only, will not overwrite
			define ('__MODEL_GEN__', __GENERATED__ . '/model' );
			define ('__META_CONTROLS__', __INCLUDES__ . '/meta_controls' ); // generated one time only, will not overwrite
			define ('__META_CONTROLS_GEN__', __GENERATED__ . '/meta_controls' );
			define ('__PLUGINS__', __GENERATED__ . '/plugins');
			define ('__FORMBASE_CLASSES__', __GENERATED__ . '/formbase_classes');

			/*
			 * Temp directories, need to be writable by web server
			 */
			...
			define ('__FORM_DRAFTS__', __SUBDIRECTORY__ . '/generated/drafts');
			define ('__PANEL_DRAFTS__', __SUBDIRECTORY__ . '/generated/drafts/panels');
			...
			define ('__DEVTOOLS__', __PHP_ASSETS__ . '/_devtools');	// developer tools, should be removed for production server
			define ('__EXAMPLES__', __PHP_ASSETS__ . '/examples'); // example site, parts of which require examples database. Should be removed for production server
			...
			//define ('__JQUERY_EFFECTS__',   'jquery/jquery-ui.1.9.js');
