...
class QDialogBase extends QDialogGen
{
	...
	/** @var bool whether to use a wrapper for the dialog content, which is necessary if you are changing
	 * the objects in the dialog using ajax.*/
...
FUNC;
	...
	public function RemoveButton ($strButtonId) {
		if (!empty($this->mixButtons)) {
			$this->mixButtons = array_filter ($this->mixButtons, function ($a) use ($strButtonId) {return $a['data-btnid'] == $strButtonId;});
		}
