<?php
	...
	/**
	 * Generated QSelect2ListBoxGen class.
	 * 
	 * This is the QSelect2ListBoxGen class which is automatically generated
	 * by scraping the JQuery UI documentation website. As such, it includes all the options
	 * as listed by the JQuery UI website, which may or may not be appropriate for QCubed. See
	 * the QSelect2ListBoxBase class for any glue code to make this class more
	 * usable in QCubed.
	 * 
	 * @see QSelect2ListBoxBase
			...
			switch ($strName) {
				case 'ContainerWidth':
					try {
						...
						if ($this->OnPage) {
							$this->CallJqUiMethod(true, 'option', 'width', $this->strContainerWidth);
						}
						break;
				...
				case 'MinimumInputLength':
					try {
						...
						if ($this->OnPage) {
							$this->CallJqUiMethod(true, 'option', 'minimumInputLength', $this->intMinimumInputLength);
						}
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
				...
				case 'MinimumResultsForSearch':
					try {
						...
						if ($this->OnPage) {
							$this->CallJqUiMethod(true, 'option', 'minimumResultsForSearch', $this->intMinimumResultsForSearch);
						}
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
				...
				case 'MaximumSelectionSize':
					try {
						...
						if ($this->OnPage) {
							$this->CallJqUiMethod(true, 'option', 'maximumSelectionSize', $this->intMaximumSelectionSize);
						}
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
				...
				case 'Placeholder':
					try {
						...
						if ($this->OnPage) {
							$this->CallJqUiMethod(true, 'option', 'placeholder', $this->strPlaceholder);
						}
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
				...
				case 'AllowClear':
					try {
						...
						if ($this->OnPage) {
							$this->CallJqUiMethod(true, 'option', 'allowClear', $this->blnAllowClear);
						}
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
				...
				case 'OpenOnEnter':
					try {
						...
						if ($this->OnPage) {
							$this->CallJqUiMethod(true, 'option', 'openOnEnter', $this->blnOpenOnEnter);
						}
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
				...
				case 'Matcher':
					try {
						...
						if ($this->OnPage) {
							$this->CallJqUiMethod(true, 'option', 'matcher', $this->mixMatcher);
						}
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
				...
				case 'FormatSelection':
					try {
						...
						if ($this->OnPage) {
							$this->CallJqUiMethod(true, 'option', 'formatSelection', $this->mixFormatSelection);
						}
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
				...
				case 'FormatResult':
					try {
						...
						if ($this->OnPage) {
							$this->CallJqUiMethod(true, 'option', 'formatResult', $this->mixFormatResult);
						}
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
				...
				case 'FormatResultCssClass':
					try {
						...
						if ($this->OnPage) {
							$this->CallJqUiMethod(true, 'option', 'formatResultCssClass', $this->mixFormatResultCssClass);
						}
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
				...
				case 'FormatNoMatches':
					try {
						...
						if ($this->OnPage) {
							$this->CallJqUiMethod(true, 'option', 'formatNoMatches', $this->mixFormatNoMatches);
						}
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
				...
				case 'FormatSearching':
					try {
						...
						if ($this->OnPage) {
							$this->CallJqUiMethod(true, 'option', 'formatSearching', $this->mixFormatSearching);
						}
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
				...
				case 'FormatInputTooShort':
					try {
						...
						if ($this->OnPage) {
							$this->CallJqUiMethod(true, 'option', 'formatInputTooShort', $this->mixFormatInputTooShort);
						}
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
				...
				case 'FormatSelectionTooBig':
					try {
						...
						if ($this->OnPage) {
							$this->CallJqUiMethod(true, 'option', 'formatSelectionTooBig', $this->mixFormatSelectionTooBig);
						}
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
				...
				case 'FormatLoadMore':
					try {
						...
						if ($this->OnPage) {
							$this->CallJqUiMethod(true, 'option', 'formatLoadMore', $this->mixFormatLoadMore);
						}
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
				...
				case 'CreateSearchChoice':
					try {
						...
						if ($this->OnPage) {
							$this->CallJqUiMethod(true, 'option', 'createSearchChoice', $this->mixCreateSearchChoice);
						}
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
				...
				case 'InitSelection':
					try {
						...
						if ($this->OnPage) {
							$this->CallJqUiMethod(true, 'option', 'initSelection', $this->mixInitSelection);
						}
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
				...
				case 'Tokenizer':
					try {
						...
						if ($this->OnPage) {
							$this->CallJqUiMethod(true, 'option', 'tokenizer', $this->mixTokenizer);
						}
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
				...
				case 'TokenSeparators':
					try {
						...
						if ($this->OnPage) {
							$this->CallJqUiMethod(true, 'option', 'tokenSeparators', $this->arrTokenSeparators);
						}
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
				...
				case 'Query':
					try {
						...
						if ($this->OnPage) {
							$this->CallJqUiMethod(true, 'option', 'query', $this->mixQuery);
						}
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
				...
				case 'Ajax':
					...
					if ($this->OnPage) {
						$this->CallJqUiMethod(true, 'option', 'ajax', $mixValue);
					}
					break;
				...
				case 'Data':
					...
					if ($this->OnPage) {
						$this->CallJqUiMethod(true, 'option', 'data', $mixValue);
					}
					break;
				...
				case 'Tags':
					...
					if ($this->OnPage) {
						$this->CallJqUiMethod(true, 'option', 'tags', $mixValue);
					}
					break;
				...
				case 'ContainerCss':
					...
					if ($this->OnPage) {
						$this->CallJqUiMethod(true, 'option', 'containerCss', $mixValue);
					}
					break;
				...
				case 'ContainerCssClass':
					...
					if ($this->OnPage) {
						$this->CallJqUiMethod(true, 'option', 'containerCssClass', $mixValue);
					}
					break;
				...
				case 'DropdownCss':
					...
					if ($this->OnPage) {
						$this->CallJqUiMethod(true, 'option', 'dropdownCss', $mixValue);
					}
					break;
				...
				case 'DropdownCssClass':
					...
					if ($this->OnPage) {
						$this->CallJqUiMethod(true, 'option', 'dropdownCssClass', $mixValue);
					}
					break;
				...
				case 'EscapeMarkup':
					try {
						...
						if ($this->OnPage) {
							$this->CallJqUiMethod(true, 'option', 'escapeMarkup', $this->mixEscapeMarkup);
						}
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
