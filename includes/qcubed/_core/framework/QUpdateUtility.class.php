<?php
	 ...
	 * It will use the QCubed Updater WebService at http://api.qcodo.com/ to perform updates to the installed
	 * QCubed system.
	 */
	class QUpdateUtility {
		...
		public function __construct($strVersion) {
			...
			$strManifestXml = $this->RetrieveFromService('GetManifest' . $strArgs, 'QCubed Version does not exist: ' . $strVersion);

			try {
				$objXml = new SimpleXMLElement($strManifestXml);
			} catch (Exception $objExc) {
				exit(sprintf("Invalid XML Response from QCubed Update WebService: %s\r\n", $strManifestXml));
			}
			...
			$strManifestXml = file_get_contents(__QCUBED_CORE__ . '/manifest.xml');
			$objXml = new SimpleXMLElement($strManifestXml);
			foreach ($objXml->files[0]->file as $objFile) {
				$strPath = (string) $objFile['path'];
				$strToken = (string) $objFile['directoryToken'];
			...
			$this->strTouchedFile[strtolower(__QCUBED_CORE__ . '/manifest.xml')] = true;
		}
		...
		public function RunUpdater($strInteractionType, $blnQuietMode) {
			...
			$this->strShouldUpdateArray['__QCUBED_CORE__|manifest.xml'] = __QCUBED_CORE__ . '/manifest.xml';
		...
		static function PrintDownloaderInstructions() {
			...
			print('Qcodo Downloader Service - ' . QCUBED_VERSION . '
...
Example:
    ' . $strCommandName . ' 0.3.4 __QCUBED__ prepend.inc.php /home/web/includes/prepend_beta3.inc.php
...
DIRECTORY_TOKEN:
    ...
    want to download (e.g. "__QCUBED__")
...
');
        ...
        Notes:
           ...
           updating to.";
			...
			printf('Qcodo Updater Service - ' . QCUBED_VERSION . '
Copyright (c) 2001 - 2009, QuasIdea Development, LLC, QCubed Project
This program is free software with ABSOLUTELY NO WARRANTY; you may
redistribute it under the terms of The MIT License.
