 ...
 * controls can add parameters to this dialog by implementing the GetMetaControlParams function.
 *
 * Everything gets saved in the configuration/codegen_options.json file.
 *
 *
 */
class QControlCodegenDlg extends QDialog {
		...
		public function __construct($objParentObject) {
			parent::__construct ($objParentObject, 'controlEditDlg');
			...
			$this->definitionOptions = array (
				new QControlParamEditor ('FormGen',
					...
					QControlParamEditor::Quote),
				new QControlParamEditor ('Name', 'Control\'s Name', QType::String),
				new QControlParamEditor ('ControlClass', 'Override of the PHP type for the control', QType::String)
			);

			// General overrides coming from QControl
			$this->generalOverrides  = array(
				new QControlParamEditor ('CssClass', 'Css Class assigned to the control', QType::String),
				new QControlParamEditor ('AccessKey', 'Access Key to focus control', QType::String),
				new QControlParamEditor ('CausesValidation', 'How and what to validate. Can also be set to a control.', QType::ArrayType,
					array(
						null=>'None',
				...
				new QControlParamEditor ('Enabled', 'Will it start as enabled (default true)?', QType::Boolean),
				new QControlParamEditor ('Required', 'Will it fail validation if nothing is entered (default depends on data definition, if NULL is allowed.)?', QType::Boolean),
				new QControlParamEditor ('TabIndex', '', QType::Integer),
				new QControlParamEditor ('ToolTip', '', QType::String),
				new QControlParamEditor ('Visible', '', QType::Boolean),
				new QControlParamEditor ('Height', 'Height in pixels. However, you can specify a different unit (e.g. 3.0 em).', QType::String),
				new QControlParamEditor ('Width', 'Width in pixels. However, you can specify a different unit (e.g. 3.0 em).', QType::String),
				new QControlParamEditor ('Instructions', 'Additional help for user.', QType::String),
				new QControlParamEditor ('Moveable', '', QType::Boolean),
				new QControlParamEditor ('Resizable', '', QType::Boolean),
				new QControlParamEditor ('Droppable', '', QType::Boolean),
				new QControlParamEditor ('UseWrapper', 'Control will be forced to be wrapped with a div', QType::Boolean),
				new QControlParamEditor ('WrapperCssClass', '', QType::String)
		...
		public function dtg_ValueRender (QControlParamEditor $objControlParam, $objParent) {
		...
		protected function LoadParams() {
			...
			if (method_exists($strControlType, 'GetMetaControlParams')) {
				$this->tabSpecific->Visible = true;
				$this->tabSpecific->SetParentControl($this->tabs);
				$this->specificOverrides = $strControlType::GetMetaControlParams();
