...
<div id="instructions">
	<h1>Using the QDataGrid2_CheckBoxColumn</h1>

	<p>In this example we will take our Paginated <strong>QDataGrid2</strong>, and add a column which has a
	...
	<p>The <strong>QDataGrid2_CheckBoxColumn</strong> is designed to help manage a column of checkboxes.
	...
	<h2>Check All</h2>
	<p>
		The <strong>QDataGrid2_CheckBoxColumn</strong> can display a checkbox in the header that will check all, or check none,
		by setting the <strong>ShowCheckAll</strong> property to true.
		In order to implement this, <strong>QDataGrid2_CheckBoxColumn</strong> must be able to get an id
		for each possible row shown, so that when the user pages through data, they can see what is checked. In order to do this,
		you must subclass <strong>QDataGrid2_CheckBoxColumn</strong> and implement the <strong>GetAllIds()</strong> method.
	</p>

	<hr>

	<p>
		In the first example, we are using the default functionality of the <strong>QDataGrid2_CheckBoxColumn</strong> class
		to create a selection list that will be acted on later. The <strong>QDataGrid2_CheckBoxColumn</strong> keeps a record
		of what is checked in the _SESSION variable. It uses a subclass of <strong>QDataGrid2_CheckBoxColumn</strong> to get
	...
	with the ACME project. To support this type of interaction, we subclass <strong>QDataGrid2_CheckBoxColumn</strong> and
