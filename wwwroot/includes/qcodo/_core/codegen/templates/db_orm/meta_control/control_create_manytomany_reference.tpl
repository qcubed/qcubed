		...
		public function <%= $strControlId %>_Create($strControlId = null) {
			...
			if ($<%= $objManyToManyReference->VariableName %>Array) foreach ($<%= $objManyToManyReference->VariableName %>Array as $<%= $objManyToManyReference->VariableName %>) {
				$objListItem = new QListItem($<%= $objManyToManyReference->VariableName %>->qFormat(), $<%= $objManyToManyReference->VariableName %>-><%= $objCodeGen->GetTable($objManyToManyReference->AssociatedTable)->PrimaryKeyColumnArray[0]->PropertyName %>);
